...
with st.sidebar:
    ...
    st.markdown(
        f'<div class="now-badge">'
        f'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#7F0303" stroke-width="2">'
    ...
    st.markdown(
        ...
        '<div>Demand is <b>never spread evenly</b> — some zones surge while others stay quiet. '
        'See where riders will be.</div></div>', unsafe_allow_html=True)

    st.markdown('<div class="side-foot">Tuned LightGBM model · trained on 2024–2025 '
...
.side-h { font-family:'Fraunces',serif; font-weight:600; font-size:1.55rem; color:var(--midnight);
  letter-spacing:-.3px; line-height:1.1; }
.side-sub { font-size:.86rem; color:var(--muted); margin-top:4px; font-weight:500; }
.side-label { font-family:'Fraunces',serif; font-weight:600; font-size:1rem; color:var(--midnight);
  margin-bottom:7px; }
.now-badge { display:flex; align-items:center; gap:12px; background:#fff; border:1px solid var(--line);
  border-left:4px solid var(--maroon); border-radius:14px; padding:13px 16px;
  box-shadow:0 8px 20px -16px rgba(15,65,74,.5); }
.now-cap { font-size:.68rem; text-transform:uppercase; letter-spacing:.7px; color:var(--muted); font-weight:700; }
.now-val { font-family:'Fraunces',serif; font-weight:600; font-size:1.12rem; color:var(--midnight); margin-top:1px; }
.side-note { font-size:.88rem; color:var(--midnight); margin-top:20px; line-height:1.55;
  display:flex; gap:10px; align-items:flex-start; font-weight:500; }
.side-note b { color:var(--maroon); font-weight:700; }
.note-ic { width:20px; height:20px; flex-shrink:0; margin-top:1px; opacity:.85; }
.side-foot { font-size:.72rem; color:var(--muted); margin-top:22px; line-height:1.5;
