...
with st.sidebar:
    ...
    mode = st.radio("Choose how to pick the day", ["By weekday", "By full date"],
    ...
    if mode == "By weekday":
        ...
        day = picked_date.weekday()
        day_name = DAYS[day]
        st.markdown(f'<div class="side-cap">{picked_date.strftime("%b %d, %Y")} · {day_name}</div>',
...
hot_zone_pct = round(hot_val / max(total_demand, 1) * 100, 1)
...
with left:
    ...
    # insight — uses the REAL leading-zone share (no invented formula)
    st.markdown(
        f'<div class="insight rise">At <b>{fmt_hour(hour)} on {day_name}</b>, '
        f'<b>{lead_boro}</b> holds <b>{lead_boro_pct}%</b> of citywide demand. '
        f'The single hottest zone is <b>{hot_name}</b> ({hot_boro}) with '
        f'<b>{hot_val:.0f} rides/hr</b> — about <b>{hot_zone_pct}%</b> of the city total. '
        f'Send drivers there first.</div>',
        unsafe_allow_html=True)
