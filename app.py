...
.rc-wrap { display:flex; flex-direction:column; height:100%; }
.rcard { position:relative; background:var(--cream); border:1px solid var(--line); border-bottom:none;
  border-radius:14px 14px 0 0; padding:16px 18px 12px; flex:1 1 auto; display:flex; flex-direction:column;
  min-height:150px; box-shadow:0 10px 22px -18px rgba(108,44,37,.45); }
...
.rc-val.txt { font-family:'Fraunces',serif; font-size:1.25rem; letter-spacing:0; line-height:1.12; }
...
def receipt_card(col, icon, label, value, sub, unit, is_text=False):
    ...
    col.markdown(
        f'<div class="rc-wrap rise"><div class="rcard"><div class="rc-top">{_svg(icon, 18)}{label}</div>'
        f'<div class="{cls}">{value}</div><div class="rc-sub">{sub}</div>'
        f'<div class="rc-spacer"></div>'
        f'<div class="rc-dash"><span>FARE EST.</span><span>{unit}</span></div></div>{ZIG}</div>',
        unsafe_allow_html=True)
...
def build_race_fig():
    ...
    race = go.Figure(
        ...
        layout=go.Layout(
            height=470, margin=dict(l=10, r=70, t=64, b=20),
            ...
            annotations=[dict(x=0.98, y=0.06, xref="paper", yref="paper", showarrow=False,
                              ...
                              font=dict(family="Fraunces", size=22, color="#6C2C25"), align="right")],
            updatemenus=[dict(type="buttons", showactive=False, x=0.0, y=1.14, xanchor="left", yanchor="bottom", direction="right", pad=dict(b=6),
...
st.sidebar.markdown('<div class="side-foot">Tuned LightGBM model · trained on 2024 to 2025 '
                    'NYC TLC trips · test MAE 3.14</div>', unsafe_allow_html=True)
